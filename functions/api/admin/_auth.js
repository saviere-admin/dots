...
export async function requireAdmin(request, env) {
  try {
    ...
    if (!token) {
      return { response: json({ ok: false, message: 'GitHub PAT is required to access the database.' }, 401) };
    }
    ...
    if (!response.ok) {
      const message = response.status === 401
        ? 'GitHub rejected this PAT. It may be expired or invalid.'
    ...
    if (githubLogin !== username) {
      return { response: json({ ok: false, message: `Unauthorized GitHub user.` }, 403) };
    }
